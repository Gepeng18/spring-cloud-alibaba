...
public class NacosPropertySourceLocator implements PropertySourceLocator {
	...
	// spring boot 启动得时候，做环境准备得时候，会调用这个函数
	// SpringBoot在启动时会准备环境,此时会调用该方法。该方法会从配置中心加载配置文件
	@Override
	public PropertySource<?> locate(Environment env) {
		...
		// 若没有设置name 与prefix属性,则要加载的配置文件名称取spring.application.name属性值
		if (StringUtils.isEmpty(dataIdPrefix)) {
	...
	private void loadApplicationConfiguration(
		...
		// 获取配置文件扩展名【配置文件bootstrap.yml中spring.cloud.nacos.config.file-extension:中配置的】
		...
		// 加载有文件名称，有扩展名，且还包含多环境选择profile的的配置文件
